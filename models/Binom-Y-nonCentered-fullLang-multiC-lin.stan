...
transformed parameters{
  ...
  for (m in 1:M){
    for (f in 1:34){
      ...
      matrix[2,2] R = multiply_lower_tri_self_transpose(R_L[f]);
      // scaled correlations
      matrix[2,2] V = quad_form_diag(R, sigma[f]);
      matrix[N[f], N[f]] C_f = C[m][ ranges[f,1]:ranges[f,2], ranges[f,1]:ranges[f,2] ];
      matrix[2*N[f], 2*N[f]] W = kronecker(V, C_f);
      matrix[2*N[f],2] dMat = designMatrix(N[f]);
      p[m][ranges[f,3]:ranges[f,4]] = (W * not_center[ranges[f,3]:ranges[f,4]]) + (dMat * z[f]);
    }
  }
}
